...
def read_parameters(folder, layer_name):
    ...
    if not os.path.exists(file_name):
        raise ValueError(
            f"Parameters file '{file_name}' not found in folder '{folder}'.")
...
#
# Parameters:
# Input     network:        List containing all the layers of the network.
# Input     folder:         Destination folder where the parameters are persisted.


def persist(network, folder):
