...
from initialization import he_initialization_weights
class FullyConnectedLayer(LayerBase):
    ...
    def __init__(self, name, input_shape, output_neurons):
        ...
        self.weights = he_initialization_weights(input_shape, output_neurons)
    ...
    def update_parameters(self, learning_rate, adam=False):
        ...
        if adam:
            parameters = {f"w{self.name}": self.weights, f"b{self.name}": self.biases}
            gradients = {f"w{self.name}": self.grad_weights, f"b{self.name}": self.grad_biases}
