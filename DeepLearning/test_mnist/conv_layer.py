...
from initialization import he_initialization_filters
...
class ConvolutionalLayer(LayerBase):
    ...
    def __init__(self, name, input_shape, num_filters, filter_size, stride, same_convolution=True, use_as_fc=False):
        self.input_shape = input_shape  # (in_channels, in_height, in_width). Note: the shape does not contain the batch size m.
        ...
        self.filters = he_initialization_filters(num_filters, in_channels, filter_size)
        self.biases = np.zeros(num_filters)

        # Create the Relu activation function
        self.activation = LeakyRelu()

        # Create a local Adam optimizer
        ...
        output_height = (self.input_shape[1] + 2 * self.padding - self.filter_size) // self.stride + 1
        output_width = (self.input_shape[2] + 2 * self.padding - self.filter_size) // self.stride + 1
    ...
    def forward(self, X):
        ...
        result = convolution(X, self.filters, self.biases, self.stride, self.padding)
    ...
    def update_parameters(self, learning_rate, adam=False):

        if self.frozen:
            return

        if adam:
            parameters = {f"w{self.name}": self.filters, f"b{self.name}": self.biases}
            gradients = {f"w{self.name}": self.grad_filters, f"b{self.name}": self.grad_biases}
            self.adam.update(parameters, gradients, learning_rate)
        else:
