...
# Implement the ConvolutionalLayer class inheriting from LayerBase
...
m = 10          # Batch size
c = 3           # Number of channels of the input examples
h = 28          # Height of the input examples
w = 28          # Width of the input examples

num_filters = 6  # Number of filters of the convolutional layer
# Filter size (the filters are square and have the same number of channels 'c' as the input examples)
filter_size = 3
stride = 1      # Stride to use in the convolutions

np.random.seed(1)  # do not modify -- the seed is fixed to 1 in the test-case!

# Define the shape of individual input examples
input_shape = (c, h, w)

# Create a convolutional layer named "CONV1", which preserves the height and width of the input
# To avoid reducing the height and width of the input examples, we must use "same convolutions"
conv1 = ConvolutionalLayer(
    "CONV1", input_shape, num_filters, filter_size, stride, same_convolution=True)

# Create a random input batch X in the range [-1, +1]
X = np.random.randn(m, c, h, w) * 2 - 1

# Perform the Forward Pass to obtain the activation A
A = conv1.forward(X)

# Create a random upstream gradient dL/dA in the range [-1, +1] -- let's call it dA for simplicity
dA = np.random.randn(*conv1.output_shape) * 2 - 1

# Perform the Backward Pass to compute dL/dW, dL/db and dL/dX
# Note: if conv1 would be an inner layer of the network, we could use dL/dX to continue the backprop process
